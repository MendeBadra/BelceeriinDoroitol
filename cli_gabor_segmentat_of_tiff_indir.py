...
# def write_separate_bands(path_dict, field_dict, field_src , field_name):
#     os.makedirs(os.path.join(os.path.dirname(path_dict[field_name]),'OrthoReflectance'), exist_ok= True)
#     for k,v in tqdm(ms_bands.items()):
#         data = field_dict[v].get()
#         print(data.shape)
#         with rasterio.open(
#             os.path.join(os.path.dirname(path_dict[field_name]),'OrthoReflectance/',f'{field_name}_transparent_mosaic_{v}.tif'),
#                 'w',
#                 driver='GTiff',
#                 height= data.shape[0],
#                 width= data.shape[1],
...
def calculate_data_frame(ortho_path, field_name, output_directory):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='This script identifies tif files in the main directory and report. Admittedly a bad solution to a particular problem with `get_gabor_segmentation` function in `gabor_texture.py`, it effectively copies the tiffs into separate bands. After that it runs the gabor_texture.py functions-written by Temuujin. The result will be saved into current working directory/CSV folder. Example: python cli_temuujin_code.py /path/to/your/dir/one/level/above/tiffs --specify_dirs ex./Medium', )
